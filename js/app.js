      ...
      runLevel(Parser.parse(plans[n]), Display)
        .then(status => {
          if (status == "lost") {
			  ...
			  my_lives--;
			  document.getElementById('lives').innerHTML = `Жизни ${my_lives}`;
			  ...
			  if (my_lives < 0) {
				  ...
				  document.getElementById('pole').style.backgroundImage = imagelevel;
			  }
			  else {
				 console.log ('stertlevel');
				 console.log (n);
            ...
            startLevel(n + 1);
				let imagelevel = `url(mbg${n+1}.jpg)`;;
				document.getElementById('pole').style.backgroundImage = imagelevel;
				if (n==plans.length-2) {
			  ...
			  if (n < plans.length - 1) {
			   ...
			   document.getElementById('pole').style.backgroundImage = "url('win.png')";
			  }
			  else {
            done();
			  }
